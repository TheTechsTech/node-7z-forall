 ...
 * @param {Boolean} override Change binary location? `True/false`.
 *
 ...
 */
module.exports = function (name, files, destination = '',
  options = {
    title: null,
    beginPrompt: null,
    progress: null,
    runProgram: null,
    directory: null
  }, type = 'gui', platform = 'win32', extension = '.exe', override = false) {
  return when.promise(function (resolve, reject, progress) {
    ...
    text = options.installPath || '';
    config.write('InstallPath=' + text + "\n");
    text = options.progress || 'no';
    config.write('Progress=' + text + "\n");
    text = options.runProgram || '';
    config.write('RunProgram=' + text + "\n");
    text = options.directory || '.' + path.sep;
    config.write('Directory=' + text + "\n");
    text = options.executeFile || '';
    config.write('ExecuteFile=' + text + "\n");
    text = options.executeParameters || '';
    ...
    if (type == 'gui')
      var sfxModule = '7zSD' + (process.platform == 'win32' ? '' : 'other32');
    else
